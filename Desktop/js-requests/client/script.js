...
axios.get('http://localhost:3000/animals').then((res) => {
    ...
    for(let i = 0; i < animals.length; i++){
        const animalLoop = document.createElement('p')
        animalLoop.textContent = animals[i]
        animalsContainer.appendChild(animalLoop)
    }
})
};
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/parmesan`).then((res) => {
...
const sendQuery = () => {
    axios.get('http://localhost:3000/query-test?i-like-eggs&favoriteFood=tacos')
...
const createFood = ((e) => {
    e.preventDefault()
    const foodInput = document.getElementById("food-input-text");
    const body = {
        newFood: foodInput.value
    }
    axios.post('http://localhost:3000/food', body)
    .then((res) => {
      const foodInputContainer = document.getElementById('food-container');
      foodInputContainer.innerHTML = ''
      res.data.forEach((element) => {
          const foodItem = document.createElement('li')
          foodItem.textContent = element
          foodItem.classList.add('string')
          foodInputContainer.appendChild(foodItem)
      })
    })
});
